            ...
            if t.to_datetime() == start_date:
                for w in get_all_wells():
                    if wwit[m, w].max(dates=get_all_timesteps()[-1]) > 0:
                        avg_wir_for_forecast = (wwit[m, w].max(
                            dates='all') - wwit[m, w, t])/last_date
                        avg_wir_hist = (wwith[m, w].max(
                            dates='all') - wwith[m, w, t])/last_date
                        match_coef = avg_wir_hist/avg_wir_for_forecast
                        last_inj = float(wwir[m, w].fix(
                            date=get_all_timesteps()[-1]))
                        inj_min = np.quantile(
                            np.array(wwir[m, w])[-36:-1], 0.00)
                        if last_inj < inj_min:
                            inj_min = last_inj
                        inj_max = np.quantile(
                            np.array(wwir[m, w])[-36:-1], 0.9)
                        if last_inj > inj_max:
                            inj_max = last_inj
                        last_bhp = float(wbhp[m, w].fix(
                            date=get_all_timesteps()[-1]))
