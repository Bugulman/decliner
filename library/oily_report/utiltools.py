...
def lang_convert(df, coll_name):
    ...
    df[coll_name] = df[coll_name].apply(
        lambda x: slugify(x, lowercase=False) if isinstance(x, str) else x
    )
...
def gtm_effect(name, gtm_date, hist_table, target_period=12):
    ...
    period = pd.date_range(
        start=gtm_date - target_period * MonthBegin(),
        periods=target_period * 2,
        freq="MS",
    )
    wellhist = hist_table[hist_table.well == name]
    df = wellhist[wellhist.date.isin(period)].groupby(wellhist.date > gtm_date).mean()
    df["well"] = name
    df["date"] = gtm_date
...
def gtm_pivot_table(frame, hist):
    ...
    effects = frame.apply(lambda x: gtm_effect(x.well, x.date, hist_table=hist), axis=1)
