...
dept_file = 'Насыщенность нефтью_wellpics_MD.inc'   #данные по насыщению с координатами x y
ijk_file = 'Насыщенность нефтью_wellpics_ijk.inc'   #данные по насыщению с координатами i j k
zone_file = 'ZONE_wellpics_ijk.inc' #данные по зонам вскрытия пластов с координатами i j k
zapas_file = r'D:\project\Minnib_project\SCHED_PATTERNS\scripts\запасы_3\сводная таблица запасов (1).xlsx'  #сводный файл с информацией по запасам
zone=['well','voronon', 'D0','D1a','D1b1','D1b2','D1b3','argil','D1v','D1g1','D1g2+3','D1d','mul_gl', 'mul_bot', 'all'] #шапка для назнавания пластов в файле с запасами
zone_dict={1:'D0', 
          2:'kin_gl', 
          3:'VIZV', 
          4:'D1a', 
			...
			for w in get_all_wells():
				if wlpth[m,w].max(dates='all') > 0 or wwith[m,w].max(dates='all') > 0:
					for c in w.connections:
						writer.writerow([c.well, c.i, c.j, c.k, float(clpr[m,c, max_time()]), float(clpt[m,c, max_time()]), float(cwir[m,c, max_time()]), float(cwit[m,c, max_time()])])

# Загрузка информации из файлов
depth = pd.read_table(dept_file, index_col=None, skiprows=4, sep=' ', names=['well','x', 'y', 'depth', 'soil'], usecols=[0,1,2,3,5])
ijk = pd.read_table(ijk_file, index_col=None, skiprows=3, sep=' ', names=['well','i', 'j', 'k', 'del', 'soil'], usecols=[0,1,2,3,5])
plast = pd.read_table(zone_file, index_col=None, skiprows=3, sep=' ', names=['well','i', 'j', 'k', 'del', 'zone'], usecols=[0,1,2,3,5])
...
merged_picks = pd.merge(left=depth, right=ijk, left_on=['well', 'soil'], right_on=['well', 'soil'])
merged_picks = pd.merge(left=merged_picks, right=plast, left_on=['well', 'i', 'j', 'k'], right_on=['well', 'i', 'j', 'k'])
...
summ = pd.merge(merged_picks.reset_index(), zapas_frame.reset_index(), on=['well', 'zone'], how='left')
...
summ.columns = ['well', 'zone', 'i', 'j', 'k', 'k2', 'lpr', 'lpt', 'wwir', 'wwit', 0, 'depth', 'depth2', 'soil']
