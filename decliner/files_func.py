...
d = {"Горизонт": ["mean"], "P заб ВНК": ["mean"], "P пласт ВНК": ["mean"]}
...
def hist_table_prepare(**kwarg):
    ...
    prod_df = pd.DataFrame(
        prod_df.groupby(["№ скваж", "Дата"])[
            "Горизонт",
            "Кол-во часов экспл всего по доб. скваж",
            "Добыча нефти за мес по скваж, т",
            "Добыча воды за мес по скваж, т",
    ...
    press_df = pd.DataFrame(
        press_df.groupby(["№ скваж", "Дата"])[
            "Горизонт", "P заб ВНК", "P пласт ВНК"
        ].agg(d)
    )
    ...
    df["QOIL"] = (
        df["Добыча нефти за мес по скваж, т"]
        / (df["Кол-во часов экспл всего по доб. скваж"])
        * 24
    )
    df["QWAT"] = (
        df["Добыча воды за мес по скваж, т"]
        / (df["Кол-во часов экспл всего по доб. скваж"])
        * 24
    )
    df["QWIN"] = (
        df["Общая закачка всего по скваж за мес, м3"]
        / (df["Кол-во часов работы всего по нагн. скваж"])
        * 24
    )
