...
def make_window(theme):
    ...
    input_layout = [                # [sg.Menu(menu_def, key='-MENU-')],
        ...
        [sg.Input(key='-INPUT-')],
        [sg.Text('Добыча', size=(13, 1)), sg.InputText(
            key='-file1-'), sg.FileBrowse()],
        [sg.Text('Давление', size=(13, 1)), sg.InputText(
            key='-file2-'), sg.FileBrowse(target='-file2-')],
...
def main():
    ...
    while True:
        ...
        if event == '-IMPORT-':
            ...
            prod = pd.read_excel(values['-file1-'], usecols="B:N")
            press = pd.read_excel(values['-file2-'], usecols="F:M")
            keyword = {'hist_file': [prod, press],
                       'gor_num': values['-GORIZON-']}
        ...
        #     print("[LOG] Dismissing Popup!")
            ...
            names = df.loc[(df.status == 'prod') & (
                df['date'] > '2010'), 'well'].unique()
            predict = pd.DataFrame(
                columns=['well', 'date', 'SOIL', 'QOIL', 'Time_x', 'Time_y', 'rate', 'month_prod'])
            to_table = pd.DataFrame(
                columns=['well', 'first_date', 'qi', 'Di', 'bi', 'Dterm'])
