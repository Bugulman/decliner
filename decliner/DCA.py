...
def declane_fit(frame):
    ...
    try:
        qi, Di = curve_fit(MH, sub['Time'], np.array(
            sub['SOIL']), bounds=(0, [100, 0.2, 0.2, 0.99]), method='trf')
    except ValueError:
        print(
    ...
    qi[3] = qi[1]*qi[3]
    logging.info(
        f'Скважина {name[0]},начало прогноза-{sub.date.min()}, qi-{round(qi[0],2)} Di-{round(qi[1],2)}, {qi}')
...
def predict_viz(df_g, start, qi, long=120):  # , last):
    ...
    plt.plot(df_g['Дата'], df_g['QOIL'], color='grey',
             alpha=0.75, label='prodaction')
    predict_range = pd.DataFrame(pd.date_range(start, periods=long, freq='MS'))
    predict = prod_predict(start, qi, long=120)
    plt.plot(predict_range, predict, 'r--',
             color='blue', alpha=0.8, label='hyperbolic')
    ...
    plt.plot(df_g['date'], np.cumsum(df_g['QOIL']*28),
             color='grey', alpha=0.75, label='prodaction')
    plt.plot(predict_range, np.cumsum(predict*28),
             color='blue', alpha=0.8, label='hyperbolic')
    plt.xlabel('месяц работы', fontsize=20)
