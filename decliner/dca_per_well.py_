...
def arps_bootstrap(t, q, size=1):
    ...
    print(
        "95% CI of initial production rate (qi) : {:.5f} to {:.5f} VOL/D".format(
            min_qi, max_qi
        )
    )
    print(
        "95% CI of initial decline rate (di)    : {:.5f} to {:.5f} VOL/D".format(
            min_di, max_di
        )
    )
    print(
        "95% CI of decline exponent (b)         : {:.5f} to {:.5f}".format(min_b, max_b)
    )
...
def remove_outlier(
    df, column_name, window, number_of_stdevs_away_from_mean, trim=False
):
    """
    ...
    df[column_name + "_rol_Av"] = (
        df[column_name].rolling(window=window, center=True).mean()
    )
    df[column_name + "_rol_Std"] = (
        df[column_name].rolling(window=window, center=True).std()
    )

    # Detect anomalies by determining how far away from the mean (in terms of standard deviation)
    df[column_name + "_is_Outlier"] = abs(
        df[column_name] - df[column_name + "_rol_Av"]
    ) > (number_of_stdevs_away_from_mean * df[column_name + "_rol_Std"])
    ...
    result = df.drop(df[df[column_name + "_is_Outlier"] == True].index).reset_index(
        drop=True
    )
